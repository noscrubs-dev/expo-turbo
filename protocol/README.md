...
Unless the closest `data-turbo` setting opts out, activation inside a Frame passes the unchanged `href`, nearest Frame ID, and optional `data-turbo-frame` value to the host-injected `FrameControllerCollection.visit` contract. That is the same registry-owned path as a programmatic Frame visit, so Frame-default/element target precedence, current/named Frames, `_self`, `_parent`, `_top`, URL admission, controller reuse, request cancellation, and response ownership cannot drift into a second React implementation. A missing named target inside a Frame falls back to the current Frame; a disabled current/named Frame or unavailable `_parent` promotes through owning-host navigation. Outside a Frame, a non-`_top` target identifying an active enabled Frame enters the shared Frame visit path and applies its target rules. Top-level `_top`, missing, and disabled targets follow ordinary top-level activation. Frame and document controllers keep independent request ownership.

Hosts may inject `navigation` into `ExpoTurboProvider` for ordinary top-level link delegation. A credential-free cross-origin HTTP(S) URL awaits `openExternal` with the normalized absolute URL. The closest `data-turbo` setting wins: exact `false` delegates a same-origin URL through `visit(url, "advance")`, delegates a cross-origin URL through `openExternal`, and a nested non-`false` setting restores normal Frame or document capture inside an opted-out ancestor. The Frame registry separately receives a `NavigationAdapter` for `_top`, promoted, and external Frame visits; the example deliberately shares one adapter between both owners. Successful delegation returns a frozen `status: "delegated"` result identifying external versus owning-host navigation. A missing or failing adapter rejects the activation Promise and never starts, supersedes, or cancels a document request.
...
Registered XML links now call this exact API for nearest-Frame and active named-Frame capture. XML form capture, Turbo root visitability, fragment/anchor behavior, arbitrary external schemes, router/history state, and exact browser `Turbo.visit(url, { frame })` behavior remain separate contracts.
