...
`useExpoTurboDocumentLink(href)` lets an app-owned registered component expose a plain top-level native link without adding React Native or a host router to the package. The hook requires the host-injected `DocumentVisitController` and returns a stable activation callback without subscribing the component to visit-state ticks. An ordinary same-origin activation delegates the unchanged `href` to that shared controller, so request headers, cancellation, response classification, and latest-visit ownership use the document GET contracts above.

Hosts may also inject `navigation` into `ExpoTurboProvider`. A credential-free cross-origin HTTP(S) URL awaits `openExternal` with the normalized absolute URL. The closest `data-turbo` setting wins: exact `false` delegates a same-origin URL through `visit(url, "advance")`, delegates a cross-origin URL through `openExternal`, and a nested non-`false` setting restores normal document loading inside an opted-out ancestor. Successful delegation returns a frozen `status: "delegated"` result identifying external versus owning-host navigation. A missing or failing adapter rejects the activation Promise and never starts, supersedes, or cancels a document request.

The activation callback is bound to the exact registered protocol node that created it. It rechecks node identity at press time and fails closed after replacement/removal even if a deterministic key is reused. Frame ancestry, fragment-bearing URLs, non-HTTP(S) or credential-bearing URLs, and disabled/target/action/method/Stream/confirmation metadata reject before fetch or navigation instead of being silently reinterpreted. Unsupported Turbo metadata still rejects on an opted-out node rather than relying on browser-only fallback behavior. The example registry supplies the actual `Pressable`, link accessibility role, pending state, styling, and visible failure handling; those remain host responsibilities.

This remains only plain top-level link ownership. Frame capture and named targets, `_top`, root-location restrictions, fragment/anchor navigation and scrolling, owning-router URL/history synchronization, explicit `mailto:`/`tel:` or other non-HTTP scheme policies, disabled/method/Stream/confirmation metadata, click/visit lifecycle events, snapshot restoration, and preload/prefetch remain unsupported. In Turbo 8.0.23, method and Stream links enter the form-submission path, so this hook does not silently reinterpret them as ordinary document GETs.
