...
describe("prepared Frame mutations", () => {
  ...
  test("morphs Frame reload children without replacing the mounted wrapper or eligible app identity", () => {
    ...
    expect(session.getNodeSnapshot(permanent.key)?.identity).toBe(permanentSnapshot?.identity)
    expect(session.getNodeSnapshot(form.key)?.morphRevision).toBe(1)
    expect(session.getNodeSnapshot(email.key)?.morphRevision).toBe(1)
    expect(session.getNodeSnapshot(permanent.key)?.morphRevision).toBe(0)
    expect(session.getNodeSnapshot(frame.key)?.morphRevision).toBe(0)
