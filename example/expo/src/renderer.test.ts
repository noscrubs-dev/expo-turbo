...
describe("React protocol renderer", () => {
  ...
  test("keeps the blank-root error when a nested mutation restores nothing renderable", async () => {
    ...
    expect(renderer.root.findAll((node) => String(node.type) === "protocol-error")).toHaveLength(1)
    // A tree-decided blank holds its verdict across the write, so the guard's
    // reset key never moves and the surface is never re-raised: one report,
    // however many nested mutations arrive. A blank the guard had to observe at
    // commit time is retried on each revision instead, and reports each
    // re-raise.
    expect(errors).toHaveLength(1)
  })
