...
- [x] App-owned plain top-level native document links through `useExpoTurboDocumentLink`, with same-origin GET admission and latest-visit ownership delegated to the observable document controller plus fail-closed stale, Frame-scoped, fragment, and unsupported-metadata activation.
...
- [ ] Remaining Turbo link/navigation behavior: Frame-scoped and named targets, `_top`, host-router URL/history integration, root-scoped visitability, fragment/anchor navigation and scrolling, disabled/method/Stream/confirmation/action metadata, explicit non-HTTP scheme policies, snapshot cache/restoration, and preload/prefetch.
...
The root package and explicit `expo-turbo/core`, `expo-turbo/adapters`, and `expo-turbo/registry` subpaths expose the current version, errors, inspector, parser, deterministic diagnostic serializer, typed logical events and state-reference resolution, addressable tree/session/state with subtree disposal, selectors, structural and custom Stream dispatch, document GET loading/visit control plus Frame target/request/recurse/controller/programmatic-visit APIs, semantic style adapters, typed component/component-action/custom-Stream-action registries, codecs, and host-adapter contracts. `expo-turbo/react` exposes the provider, root renderer, typed component-action/document/scoped-state/style/disposal hooks, top-level document-link activation and delegated results through an optional navigation adapter, explicit form state boundaries, automatic Frame state inheritance, host-defined document and Frame boundaries, nearest-document/Frame binding hooks, node and lifecycle subscription hooks, and error surface. `expo-turbo/testing` remains a reserved module boundary and intentionally exports no runtime APIs yet. Deep source imports are unsupported.
