...
- [x] Plain current-document `refresh` Streams with ignored target/template content, Turbo's 150 ms trailing debounce, exact active-URL and active-visit ownership guards, a session-shared latest-20 request-ID suppression set populated by document, Frame/recurse, and canonical form requests, and complete document replacement through the existing GET lifecycle. Morph/scroll refresh policies remain unsupported.
