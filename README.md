...
- [x] Turbo 8.0.23-compatible confirm-only native link activation: `data-turbo-confirm` alone is inert during activation and follows the same document, Frame, delegated, disabled, and request-supersession paths as a plain link. Functional link confirmation remains part of the unsupported method/Stream-generated form-link path; confirm-based prefetch suppression remains pending with preload/prefetch support.
...
- [x] Exact Turbo 8.0.23 link-owned action recognition: only lowercase `advance`, `replace`, and `restore` are actions. Present blank, invalid, differently cased, and whitespace-padded values are non-actions that retain the ordinary document, Frame, or delegated path and explicitly mask destination-Frame inheritance. Browser-bypassed scheme, cross-origin/root-delegated, and opted-out paths ignore action metadata; exact `advance` works for visitable top-level links and Frame links promoted to the owning document. Visitable `replace`/`restore` and Frame-local recognized actions fail closed before request ownership until history/snapshot support exists; disabled presence still wins first.
- [x] Exact destination-Frame action inheritance admission for registered native links and programmatic Frame visits: when the caller supplies no action, the resolved destination Frame contributes only an exact lowercase `advance`, `replace`, or `restore`; invalid Frame values remain ordinary loads, explicit link non-actions mask inheritance, and `_top`/external paths do not inherit. Recognized Frame-local inherited actions fail closed before fetch until native Frame history promotion exists.
...
- [ ] Remaining Turbo link/navigation behavior: host-router URL/history synchronization, explicit `replace`/`restore` document actions, applying inherited/explicit Frame actions through Frame-local history promotion, fragment/anchor navigation and scrolling, method/Stream-generated form links with functional confirmation, native download/non-current browsing-context adapters, snapshot cache/restoration, and preload/prefetch.
