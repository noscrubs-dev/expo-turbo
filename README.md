...
- [x] Exact Turbo 8.0.23 link-owned action recognition: only lowercase `advance`, `replace`, and `restore` are actions. Present blank, invalid, differently cased, and whitespace-padded values are non-actions that retain the ordinary document, Frame, or delegated path and mask Frame-level inheritance; absent link metadata retains the current path only where no inherited Frame action applies. Browser-bypassed scheme, cross-origin/root-delegated, and opted-out paths ignore action metadata; exact `advance` works for visitable top-level links and Frame links promoted to the owning document. Visitable `replace`/`restore` and Frame-local recognized actions fail closed before request ownership until history/snapshot support exists; disabled presence still wins first.
...
- [ ] Remaining Turbo link/navigation behavior: host-router URL/history synchronization, explicit `replace`/`restore` document actions, Frame action inheritance and Frame-local history promotion, fragment/anchor navigation and scrolling, method/Stream-generated form links with functional confirmation, native download/non-current browsing-context adapters, snapshot cache/restoration, and preload/prefetch.
