...
- [x] Destination-aware form request ownership through `FormSubmissionController`: exact proposals are re-admitted before ownership, document and exact-Frame lanes are shared with GET loaders across instances, pre-admission failures cannot cancel an incumbent, same-destination and same-form destination-change requests supersede safely, distinct Frames remain concurrent, and the exact request is fetched once.
...
- [ ] Form-submission lifecycle and presentation: duplicate prevention, pending submitter/loading/accessibility, live announcements, registered terminal error/retry surfaces, and physical accessibility evidence.
