...
- [x] Explicit native successful-entry metadata for semantic UTF-8 `_charset_` descriptors and value-descriptor direction companions: charset names match ASCII-insensitively while preserving authored casing, ordinary values named `_charset_` stay ordinary, and a nonempty host-admitted companion name plus resolved `ltr`/`rtl` is emitted immediately after its owning value without browser control discovery or direction inference.
...
- [ ] Remaining successful-control semantics: automatic XML/DOM control discovery, browser select defaulting and option-text value derivation, files, image coordinates, browser `_charset_` discovery/hidden-input eligibility, automatic or inherited `dirname` resolution and complete eligible-control coverage, and form-associated custom elements.
