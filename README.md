...
- [x] Destination-aware form transport through `FormSubmissionController`: exact proposals are re-admitted before ownership, document and exact-Frame lanes are shared with GET loaders across instances, pre-admission failures cannot cancel an incumbent, same-destination and same-form destination-change requests supersede safely, distinct Frames remain concurrent, and the exact request is fetched once into a frozen destination-bearing unapplied candidate. Coordination changes neither the tree nor pending UI.
...
- [ ] Form response disposition and application: matching-Frame extraction/commit, Turbo Stream dispatch, `204`/blank-`201` completion, redirected canonical GET, unsafe top-level response enforcement, typed terminal error policy, and authoritative `422` rendering/state preservation.
