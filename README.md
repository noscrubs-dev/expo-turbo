...
- [x] Explicit value-descriptor direction companions: a nonempty host-admitted companion name plus resolved `ltr`/`rtl` is emitted immediately after its owning value, including an empty primary value, without browser control discovery or direction inference.
...
- [ ] Remaining successful-control semantics: automatic XML/DOM control discovery, browser select defaulting, files, image coordinates, automatic or inherited `dirname` resolution and complete eligible-control coverage, and form-associated custom elements.
